...
window.CRM_APP = {
    ...
    refreshCurrentView: async function(successMessage = '操作成功！正在刷新...') {
        ...
        // Don't show success message here, authedFetch will handle it.
        ...
        // 2. 標記頁面需要重新載入資料 (邏輯不變)
        if (!currentPageName.includes('-details') && currentPageName !== 'weekly-detail' && window.CRM_APP.pageConfig[currentPageName]) {
            window.CRM_APP.pageConfig[currentPageName].loaded = false;
             console.log(`[Refresh] Marked list page ${currentPageName} as not loaded.`);
        }
        if (window.CRM_APP.pageConfig.dashboard) {
            window.CRM_APP.pageConfig.dashboard.loaded = false; // Keep marking dashboard as needing reload
             console.log(`[Refresh] Marked page dashboard as not loaded.`);
        }
         if ((currentPageName === 'weekly-detail' || currentPageName === 'weekly-business') && window.CRM_APP.pageConfig['weekly-business']) {
            window.CRM_APP.pageConfig['weekly-business'].loaded = false;
            console.log(`[Refresh] Marked weekly-business list page as not loaded.`);
         }
...
async function loadHTMLComponents() { /* ... (Logic remains the same) ... */
    ...
    try {
        ...
        const templatePromises = formTemplates.map(type => {
            const templateFileName = `event-form-${type === 'dx' ? 'general' : type}.html`;
